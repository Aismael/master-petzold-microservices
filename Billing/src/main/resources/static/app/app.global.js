    ...
    function Inlay(sanitizerArg, router, route, loginService, fromOrderService, errorService) {
        ...
        this.router.events.subscribe(function (event) {
            if (event instanceof router_1.NavigationEnd) {
                _this.bill = event.urlAfterRedirects.includes("bill");
                _this.pay = event.urlAfterRedirects.includes("pay");
                _this.make = event.urlAfterRedirects.includes("make");
            }
        });
        this.route.queryParams.subscribe(function (params) {
            if (!loginService.getLast()) {
                if (!params["accountId"]) {
                    _this.errorService.sendMessage("accountId");
                }
                else {
            ...
            if (!fromOrderService.getLast()) {
                if (!params["orderId"]) {
                    _this.errorService.sendMessage("orderId");
                }
                else {
